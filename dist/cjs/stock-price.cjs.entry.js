...
const StockPrice = class {
  ...
  fetchStockPrice(stockSymbol) {
    this.loading = true;
    fetch(`https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol=${stockSymbol}&apikey=${global.API_KEY}`)
      .then(res => {
      return res.json();
    })
      .then(parseRes => {
  ...
  __stencil_render() {
    ...
    if (this.fetchPrice) {
      dataContent = index.h("p", null, "Price: $", this.fetchPrice);
    }
    if (this.loading) {
      dataContent = (index.h("div", { class: "lds-ripple" }, index.h("div", null), index.h("div", null)));
    }
    return [
