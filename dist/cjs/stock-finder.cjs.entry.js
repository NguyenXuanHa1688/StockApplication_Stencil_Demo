...
const global = require('./global-0b496e4b.js');
...
const StockFinder = class {
  ...
  onFindStock(event) {
    ...
    fetch(`https://www.alphavantage.co/query?function=SYMBOL_SEARCH&keywords=${stockName}&apikey=${global.API_KEY}`)
      .then(res => res.json())
      .then(parseRes => {
  ...
  __stencil_render() {
    let content = index.h("ul", null, this.searchResult.map(result => (index.h("li", { onClick: this.onSelectSymbol.bind(this, result.symbol) }, index.h("strong", null, result.symbol), " - ", result.name))));
    if (this.loading) {
      content = (index.h("div", { class: "lds-ripple" }, index.h("div", null), index.h("div", null)));
    }
    if (this.error) {
      content = index.h("p", null, this.error);
    }
    return [
      index.h("form", { onSubmit: this.onFindStock.bind(this) }, index.h("input", { id: "stock-symbol", ref: el => (this.stockNameInput = el) }), index.h("button", { type: 'submit' }, "Find")),
      content
    ];
  }
...
//# sourceMappingURL=stock-finder.cjs.entry.js.map